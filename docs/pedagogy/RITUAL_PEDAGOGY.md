...
3. receber dados;
4. escrever código na linguagem declarada pela rota fora do jogo;
5. executar;
6. informar a saída;
...
## 10. Contrato autoral obrigatório

Casos com `ritualContractVersion` devem declarar, em cada desafio: fenômeno, instrumento, origem dos dados, significado da entrada, ação dependente, risco, operação lógica, saída, consequência física e conhecimento de Grimório.

O contrato é validado antes do conteúdo entrar no catálogo. Ele impede que um exercício pronto receba apenas uma aparência gótica depois.

## 11. Linguagem e biblioteca

O problema semântico pertence ao ritual. Sintaxe, exemplo, template e instruções pertencem ao adaptador de linguagem.

- Java e Python podem resolver o mesmo problema quando a rota os suporta.
- MicroPython/Pybricks começa por lógica pura ou APIs simuladas.
- tarefas dependentes de motor/sensor precisam de hub real, simulação explícita ou validação do mentor; nunca fingir que a VPS reproduziu o hardware.
